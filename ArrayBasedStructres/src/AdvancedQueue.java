...
public class AdvancedQueue<T> {
    ...
    public String toString() {
        ...
        for (int i = 0; i < size; i++) {
            int index = (start + i);
            str.append(elements[index % size]);
            if (index % size != end - 1) {
                str.append(", ");
            }
        }
...
//        }
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof AdvancedQueue<?>)) {
            return false;
        }

        AdvancedQueue<?> queue = (AdvancedQueue<?>)obj;
        if (this.size != queue.size) {
            return false;
        }
        
        for (int i = 0; i < size; i++) {
            if (!this.elements[(this.start + i) % this.size].equals(queue.elements[(queue.start + i) % queue.size])) {
                return false;
            }
        }
        return true;
